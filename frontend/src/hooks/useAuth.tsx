  ...
  get: async (forceRefresh = false): Promise<any> => {
    ...
    activeProfileRequest = (async () => {
      try {
        ...
        // ✅ FALLBACK: Usa cache em erro de rede, mas NÃO em 401
        if (profileCache && !forceRefresh && error.response?.status !== 401) {
          if (import.meta.env.DEV) console.log("⚡ Usando cache do profile devido a erro de rede");
          return profileCache;
        }
        throw error;
      } finally {
...
export interface User {
  ...
  is_staff?: boolean; // ✅ Para verificação de admin
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const refreshToken = async (): Promise<string | null> => {
    ...
    try {
      ...
      if (import.meta.env.DEV) console.warn("❌ Falha ao renovar token:", {
  ...
  const handleLogout = useCallback((shouldNavigate = true) => {
    ...
    // ✅ Navega com replace para evitar histórico inseguro
  ...
  const initializeAuth = useCallback(async () => {
    // ✅ Guarda contra múltiplas execuções
    ...
    if (storedUser) {
      try {
        ...
        if (import.meta.env.DEV) console.warn("⚠️ Erro ao parsear usuário salvo, limpando...");
        localStorage.removeItem("auth_user");
      }
    }
    ...
    try {
      ...
      if (profileData) {
        const userData: User = {
          ...
          // ✅ Garante que is_staff sempre exista
          is_staff: profileData.is_staff ?? false
        };
        setUser(userData);
        localStorage.setItem("auth_user", JSON.stringify(userData));
        if (import.meta.env.DEV) console.log("✅ Profile carregado:", { id: userData.id, email: userData.email });
      }
    ...
    try {
      ...
      try {
        ...
        const userData: User = {
          ...
          ...profileData,
          is_staff: profileData.is_staff ?? false // ✅ Garante is_staff
        };
        localStorage.setItem("auth_user", JSON.stringify(userData));
        if (import.meta.env.DEV) console.log("🔍 [DEBUG] Profile atualizado:", { id: userData.id });
        setUser(userData);
      } catch (e) {
        // Fallback mínimo para não travar a UI
        if (import.meta.env.DEV) console.warn("⚠️ Perfil não carregado no login, usando dados básicos");
        setUser({ id: 0, email, name: email.split('@')[0], is_staff: false });
      }
    } catch (error: any) {
      // ✅ Limpa estado em caso de erro crítico no login
      handleLogout(false);
      if (import.meta.env.DEV) console.error("❌ Erro no login:", {
        message: error?.message,
        status: error?.response?.status
      });
      throw error;
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      try {
        ...
        if (import.meta.env.DEV) console.warn("⚠️ Perfil não carregado no login Google, usando dados básicos");
        setUser({ id: 0, email: response.data.email, name: response.data.name || "Consultora", is_staff: false });
      }
    } catch (error: any) {
      if (import.meta.env.DEV) console.error("❌ Erro Google Sign-In:", {
        message: error?.message,
        status: error?.response?.status
      });
  ...
  const signInDemo = () => {
    const demoUser: User = { 
      ...
      plan: "FREE",
      is_staff: false // ✅ Define explicitamente
    };
    setUser(demoUser);
  ...
  const refreshProfile = async () => {
    try {
      ...
      if (import.meta.env.DEV) console.error("❌ Erro ao atualizar profile:", error);
    }
  };
...
export const useAuth = () => {
  ...
  if (!context) {
    throw new Error("useAuth deve ser usado dentro de um AuthProvider. Verifique se <AuthProvider> envolve sua árvore de componentes.");
  }
  return context;
};