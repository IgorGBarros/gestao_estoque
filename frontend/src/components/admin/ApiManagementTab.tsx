...
const ENDPOINTS = [
  ...
  {
    path: "/api/public/theme/", method: "GET",
    description: "Configuração de tema visual público",
    auth: "Público", rate_limit: "30 req/min",
    pricing_tier: ["starter", "pro", "enterprise"],
    sample: { primary_color: "#2D5F3F", font_family: "DM Sans" },
  },
  ...
  },
  {
    ...
    description: "Métricas globais do sistema (totais agregados, sem dados pessoais)",
    auth: "API Key + Scope", rate_limit: "30 req/min",
    pricing_tier: ["enterprise"],
    sample: { total_products: 8420, total_movements_30d: 124800 },
  },
  ...
  {
    path: "webhook → price.changed", method: "POST",
    description: "Notifica mudanças de preço oficial de marca",
    auth: "Webhook Secret", rate_limit: "ilimitado",
    pricing_tier: ["pro", "enterprise"],
    sample: { event: "price.changed", barcode: "7891234567890", old: 45.9, new: 49.9 },
  },
];
...
export default function ApiManagementTab({ formatCurrency, toast }: Props) {
  ...
  const [newWebhook, setNewWebhook] = useState({ name: "", url: "", events: ["price.changed"] });
  const [createdSecret, setCreatedSecret] = useState<string | null>(null);

  const generateSecret = () =>
    "pk_live_" + Array.from({ length: 32 }, () => Math.random().toString(36).charAt(2)).join("");
  ...
  const handleRotate = (id: string) => {
    setApiKeys(apiKeys.map(k => k.id === id ? { ...k, key_prefix: generateSecret().slice(0, 16) } : k));
  ...
  const handleCreateWebhook = () => {
    ...
    setWebhooks([...webhooks, {
      id: "wh_" + Date.now(), name: newWebhook.name, url: newWebhook.url,
      events: newWebhook.events, active: true,
      stats: { delivered_24h: 0, failed_24h: 0, avg_latency_ms: 0 },
    }]);
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => toast({ title: "Analytics em breve" })}
                      className="p-1.5 hover:bg-secondary rounded" title="Ver uso">
                      <BarChart3 className="h-4 w-4" />
                    </button>
                    <button onClick={() => handleRotate(key.id)}
                      className="p-1.5 hover:bg-secondary rounded" title="Rotacionar">
                      <RefreshCw className="h-4 w-4" />
                    </button>
                    <button onClick={() => handleRevoke(key.id)}
                      className="p-1.5 text-destructive hover:bg-destructive/10 rounded" title="Revogar">
            ...
            <div key={w.id} className="p-4 border border-border rounded-lg hover:border-primary/50 transition-colors">
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex items-center gap-2 ml-2">
                  ...
                  <button onClick={() => toggleWebhook(w.id)} className="p-1 hover:bg-secondary rounded">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {PRICING_TIERS.map(plan => (
            <div key={plan.tier}
              ...
              {plan.popular && <Badge className="mb-3">Mais Popular</Badge>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => { setShowKeyModal(false); setCreatedSecret(null); }} className="p-2 hover:bg-secondary rounded-lg">
                <X className="h-4 w-4" />
              </button>
            </div>
            {createdSecret ? (
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => { navigator.clipboard.writeText(createdSecret); toast({ title: "Copiado!" }); }}
                    className="p-2 bg-primary text-primary-foreground rounded">
                    <Copy className="h-4 w-4" />
                  </button>
                </div>
                <button onClick={() => { setShowKeyModal(false); setCreatedSecret(null); }}
                  className="w-full py-2 bg-primary text-primary-foreground rounded-lg">Concluir</button>
              </div>
            ) : (
              <div className="space-y-4">
                <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowWebhookModal(false)} className="p-2 hover:bg-secondary rounded-lg">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-4">
