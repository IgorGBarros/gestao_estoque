...
// erro já nasce escalado) + Central de Ajuda (vídeo, FAQ, guia, novidade),
// agrupada por categoria, consumindo o MESMO endpoint que a seção
// "Aprenda a usar" do profile usa — zero conteúdo duplicado.
import { useState, useEffect, useRef } from "react";
import { useNavigate } from "react-router-dom";
import { ArrowLeft, Send, MessageCircle, Video, Plus, Loader2, Sparkles, User, ShieldCheck, PlayCircle, HelpCircle, BookOpen, Newspaper } from "lucide-react";
...
export default function Support() {
  ...
  const [aba, setAba] = useState<"chat" | "conteudo">("chat");
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <header className="sticky top-0 z-20 border-b border-border bg-card">
        ...
        <div className="mx-auto flex max-w-3xl gap-1 px-4 pb-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
              aba === "conteudo" ? "bg-brand text-white" : "text-muted-foreground hover:bg-secondary"
            }`}
          >
      ...
      <main className="mx-auto max-w-3xl px-4 py-5">
        ...
        {aba === "conteudo" && (
          <ConteudoTab conteudos={conteudos} filtroTipo={filtroTipo} onFiltroTipo={setFiltroTipo} onAbrirItem={setItemAtivo} />
        )}
      </main>
...
// ─────────────────────────────────────────────────────────────
// Aba de conteúdo — Central de Ajuda agrupada por categoria, com filtro
// por tipo. Mesmo endpoint (GET /api/ajuda/) que o profile usa.
// ─────────────────────────────────────────────────────────────
function ConteudoTab({ conteudos, filtroTipo, onFiltroTipo, onAbrirItem }: {
  conteudos: HelpContentItem[];
  filtroTipo: string;
  onFiltroTipo: (t: string) => void;
  onAbrirItem: (item: HelpContentItem) => void;
}) {
